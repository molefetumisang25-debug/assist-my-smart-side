...
function enforceLimit(prefix: string, limit = 10, windowMs = 60_000) {
  const rl = checkRateLimit(clientKey(prefix), limit, windowMs);
  if (!rl.ok) {
    throw new Response("Too many requests. Please slow down.", {
      status: 429,
...
export const generateResearch = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    enforceLimit("research", 5, 60_000);
...
export const generatePlan = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    enforceLimit("planner", 10, 60_000);
...
export const documentAction = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    enforceLimit("documents", 10, 60_000);
    const systems: Record<string, string> = {
